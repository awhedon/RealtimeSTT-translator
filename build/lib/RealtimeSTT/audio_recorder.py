...
class AudioToTextRecorder:
                ...
                if use_groq:
                    ...
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
                        ...
                        with wave.open(temp_filename, 'wb') as wf:
                            ...
                            wf.writeframes((audio_copy * INT16_MAX_ABS_VALUE).astype(np.int16).tobytes())
                    ...
                    print('dir(transcription):', dir(transcription))
                    print('transcription:', transcription)
                    self._set_state("inactive")
                    transcription_text = transcription.text
                    self.detected_language = 'en' # Groq doesn't provide this, so we assume English
                    self.detected_language_probability = 1.0  # Groq doesn't provide this, so we assume 100% confidence
    ...
    def _realtime_worker(self):
        ...
        The method is responsible for transcribing recorded audio frames
        ...
        and a callback function is invoked with this text if specified.
        """
            ...
            while self.is_running:
                if self.is_recording:
                    ...
                    if use_groq:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
                            ...
                            with wave.open(temp_filename, 'wb') as wf:
                                ...
                                wf.setsampwidth(2)
                                wf.setframerate(16000)
                                wf.writeframes((audio_array * INT16_MAX_ABS_VALUE).astype(np.int16).tobytes())
                        ...
                        try:
                            ...
                            realtime_text = transcription.text
                            self.detected_realtime_language = 'en' # Groq doesn't have this, so we assume English
                            self.detected_realtime_language_probability = 1.0 # Groq doesn't have this, so we assume 100% confidence
                        except Exception as e:
                            logging.error(f"Error in Groq transcription: {str(e)}")
                            continue
                        finally:
                            os.unlink(temp_filename)

                    elif self.use_main_model_for_realtime:
                        print('using main model for realtime')
                        with self.transcription_lock:
                            try:
                    ...
                    if self.is_recording and time.time() - self.recording_start_time > 0.5:
                        logging.debug('Processing realtime transcription')
                        ...
                        if len(self.text_storage) >= 2:
                            ...
                            prefix = os.path.commonprefix(last_two_texts)
                        ...
                        if matching_pos < 0:
                            stabilized_text = (self.realtime_stabilized_safetext or self.realtime_transcription_text)
                        else:
                            stabilized_text = self.realtime_stabilized_safetext + self.realtime_transcription_text[matching_pos:]

                        self._on_realtime_transcription_stabilized(
                            self._preprocess_output(stabilized_text, True)
                        )
